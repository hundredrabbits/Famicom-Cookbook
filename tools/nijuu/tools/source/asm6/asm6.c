...
#define NOORIGIN 0xc0000000//nice even number so aligning works before origin is defined
...
typedef struct {
	...
	int value;				//PC (label), value (equate), param count (macro), funcptr (reserved)
...
void *rsvdlist[]={		 //all reserved words
		...
		"",nothing,	//'nothing' marks end of opcodes
...
char *Incomplete="Incomplete expression.";
...
char *listerr=0;//error message for list file
...
void initlabels() {
	...
	labels=1;
	labellist=(label**)malloc(INITLISTSIZE*sizeof(label*));
	...
	do {//opcodes first
		...
		(*p).type=RESERVED;
		i+=2;
	} while(rsvdlist[i+1]!=nothing);

	do {//other reserved words now
		findlabel(rsvdlist[i]);
		p=newlabel();
		(*p).name=rsvdlist[i];
		(*p).value=(int)rsvdlist[i+1];
		(*p).type=RESERVED;
		i+=2;
	} while(rsvdlist[i]);
	lastlabel=p;
...
void growlist() {
	...
	tmp=(label**)malloc(maxlabels*sizeof(label*));
...
void processline(char *src,char *errsrc,int errline) {
	...
	if(errmsg) {	//expandline error?
		...
		if(makemacro) {	//we're inside a macro definition
			...
			if(p) if((*p).value==(int)endm) {
				comment=0;
				if(endmac)
					*(short*)endmac='\n';//hide "ENDM" in case of "label: ENDM"
				else
			...
			if(makemacro>(char**)1) {
				...
				*makemacro=malloc(strlen(line)+sizeof(char*)+1);
		...
		if(reptcount) {//REPT definition is in progress?
			...
			if(reptcount || endmac) {	//add this line to REPT body
				if(comment)
					strcat(line,comment);		//keep comment for listing
				*makerept=malloc(strlen(line)+sizeof(char*)+1);
		...
		if(skipline[iflevel]) {//conditional assembly.. no code generation
			...
			if((*p).value!=(int)_else && (*p).value!=(int)elseif && (*p).value!=(int)endif
			&& (*p).value!=(int)_if && (*p).value!=(int)ifdef && (*p).value!=(int)ifndef)
				break;
		}
		if(!p) {//maybe a label?
...
int main(int argc,char **argv) {
	...
	if(!outputfilename) {
		strcpy(nameptr,".bin");
		outputfilename=strdup(str);
	}

	if(listfilename==(char*)1) {	//if listfile was wanted but no name was specified, use srcfile.LST
		strcpy(nameptr,".lst");
		listfilename=strdup(str);
	}
...
void listline(char *src,char *comment) {
	...
	if(oldpass!=pass) {//new pass = new listfile
		...
		if(!listfile) {
			...
			puts("Can't create list file.");//not critical, just give a warning
			return;
		}
...
void include(label *id,char **next) {
	...
	f=fopen(np,"r+t");	//read as text, the + makes recursion not possible
	if(!f) {
		errmsg=CantOpen;
		error=1;
	} else {
...
void opcode(label *id, char **next) {
	...
	for(op=(*id).line;*op!=0xff;op+=2) {//loop through all addressing modes for this instruction
...
void ifdef(label *id,char **next) {
	...
	if(iflevel>=IFNESTS-1)
		...
		iflevel++;
	getlabel(s,next);
	skipline[iflevel]=!(int)findlabel(s) || skipline[iflevel-1];
	ifdone[iflevel]=!skipline[iflevel];
}
...
void macro(label *id, char **next) {
	...
	makemacro=(char**)1;//flag for processline to skip to ENDM
	if(errmsg) {//no valid macro name
		...
		while(getlabel(word,&src)) {//don't affect **next directly, make sure it's a good name first
			*next=src;
			*makemacro=malloc(strlen(word)+sizeof(char*)+1);
			makemacro=(char**)*makemacro;
